...
use eosio_shipper_gf::shipper_types::{
    BlockHeader, BlockPosition, ProducerKey, ProducerSchedule, SignedBlock, Transaction,
    TransactionReceiptV0,
};
...
pub async fn parse_new_block(
    ...
    prev_block: &Option<BlockPosition>,
) {
    match block {
        SignedBlock::signed_block_v0(b) => {
            let total_cpu = b
                .transactions
                .par_iter()
                .map(|t| t.header.cpu_usage_us)
                .sum();
            let total_net = b
                .transactions
                .par_iter()
                .map(|t| t.header.net_usage_words)
                .sum();
            start_async(
                total_cpu,
                total_net,
