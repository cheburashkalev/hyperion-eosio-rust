...
use eosio_shipper_gf::shipper_types::{
    Account, BlockPosition, GetBlocksResultV0Ex, ShipResultsEx, SignedBlock, TableRowTypes,
};
...
pub async fn start_index_block_result_v0(start_block: &u32) -> Result<(),String> {
    println!("Init Indexing");
    // Prepare configs
    let ship_config = configs::ship::get_ship_con_config();

    println!("Init SHIP connection");
    let ws_stream = connect_async(ship_config.url.clone()).await.unwrap().0;
    ...
    shipper_abi
        .set_abi_json("0", abi_config.clone())
        .unwrap_or_else(|e| {
            shipper_abi.destroy();
    ...
    while let Some(message) = read.next().await {
        match message {
            Ok(Message::Text(text)) => {
                ...
                shipper_abi
                    .set_abi_json(EOSIO_SYSTEM, (&msg_texts).to_string())
                    .unwrap_or_else(|e| {
                        shipper_abi.destroy();
                        panic!("Error create shipper abi: {:?}", e);
                    });
                let status_request = GetStatusRequestV0::new();
                let request_bytes = status_request.marshal_binary().unwrap();
                tx.send(request_bytes).unwrap();
            }
            Ok(Message::Binary(data)) => {
                parse_ship(shipper_abi, &data, &semaphore, &start_block, &tx).await;
                continue;
            }
        ...
        Ok(msg) => {
            match msg {
                ShipResultsEx::BlockResult(BlockResult) => {
                    ...
                    index_block::parse_new_block(
                        block,
                        block_ts,
                        this_block,
                        &BlockResult.prev_block,
                    );
                    ...
                    let header = match block {
                        SignedBlock::signed_block_v0(b) => &b.signed_header.header,
                        SignedBlock::signed_block_v1(b) => &b.signed_header.header,
                    };
                    index_action::parse_new_action(
                        shipper_abi,
                        semaphore.clone(),
                        header,
                        BlockResult.traces,
...
async fn draw_progress_bar(current: u32, max: u32) {
    ...
    print!(
        ...
        0,
        0,
        0 //operations_per_second as u32
    );
